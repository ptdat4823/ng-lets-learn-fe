...
export class ReportService {
  ...
  private getToDoItemsForCourse(courseId: string): Promise<ToDoItem[]> {
    return this.courseService.getCourseReports(courseId)
  ...
  private async getToDoItemsForAllCourses(): Promise<ToDoItem[]> {
    try {
      ...
      let courses: any[] = [];
      courses = await this.courseService.getUserCourses();
      ...
      const allReports = await Promise.all(
        courses.map((course: any) => 
          this.courseService.getCourseReports(course.id)
            .then((reports: any) => {
        ...
        courses.map((course: any) => 
          this.courseService.getCourseReports(course.id)
            ...
            .catch((error: any) => {
              ...
              // Return empty reports instead of failing completely
              return { 
                courseName: course.title, 
                assignmentReport: { assignments: [] }, 
                quizReport: { quizzes: [] } 
