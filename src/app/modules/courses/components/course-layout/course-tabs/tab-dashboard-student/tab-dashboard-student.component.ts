...
export class TabDashboardStudentComponent implements OnInit {
  ...
  async fetchReports() {
    ...
    for (const m of months) {
      ...
      try {
        const quiz = await GetQuizOverallReport(this.course.id, s.toISOString(), e.toISOString());
        ...
        if (quiz?.singleQuizReports?.length) {
          for (const q of quiz.singleQuizReports) {
            const userMarkObj = q.studentWithMarkOver8?.find((stu: any) => stu.student.id === this.currentUser?.id)
              || q.studentWithMarkOver5?.find((stu: any) => stu.student.id === this.currentUser?.id)
              || q.studentWithMarkOver2?.find((stu: any) => stu.student.id === this.currentUser?.id)
              || q.studentWithMarkOver0?.find((stu: any) => stu.student.id === this.currentUser?.id);
        ...
        quizMarks.push(userQuizMarks.length ? userQuizMarks.reduce((a, b) => a + b, 0) / userQuizMarks.length : 0);
      } catch {
        quizMarks.push(0);
      }
      try {
        const assignment = await GetAssignmentOverallReport(this.course.id, s.toISOString(), e.toISOString());
        ...
        // Average for this month (for this user)
        assignmentMarks.push(userAssignmentMarks.length ? userAssignmentMarks.reduce((a, b) => a + b, 0) / userAssignmentMarks.length : 0);
      } catch {
        assignmentMarks.push(0);
      }
    }
    this.avgQuizMarkSegments = [{ label: 'Quiz', data: quizMarks.map(v => v * 100), color: '#EC4899' }];
    this.avgAssignmentsMarkSegments = [{ label: 'Assignments', data: assignmentMarks.map(v => v * 100), color: '#A855F7' }];
  ...
  populateStats() {
    ...
    this.assignmentsStats = [
      { label: 'Total assignments', value: this.getUserAssignmentCount().toString() },
      { label: 'Assignments to do', value: this.getUserAssignmentToDoCount().toString() },
      { label: 'Next assignment will end on', value: this.assignmentReport?.closestNextEndAssignment ? this.formatDate(this.assignmentReport.closestNextEndAssignment) : '-' },
    ];
    ...
    this.topAssignmentsMarkSegments = (this.assignmentReport?.singleAssignmentReports || [])
      .map((a, idx) => {
  ...
  getQuizAvgMark(): number {
    if (!this.quizReport || !this.quizReport.singleQuizReports?.length) return 0;
    ...
    for (const quiz of this.quizReport.singleQuizReports) {
      // Find the student's mark for this quiz
      const userMarkObj = quiz.studentWithMarkOver8?.find((stu: any) => stu.student.id === this.currentUser?.id)
        || quiz.studentWithMarkOver5?.find((stu: any) => stu.student.id === this.currentUser?.id)
        || quiz.studentWithMarkOver2?.find((stu: any) => stu.student.id === this.currentUser?.id)
        || quiz.studentWithMarkOver0?.find((stu: any) => stu.student.id === this.currentUser?.id);
      if (userMarkObj && typeof userMarkObj.mark === 'number') {
  ...
  getAssignmentAvgMark(): number {
    if (!this.assignmentReport || !this.assignmentReport.singleAssignmentReports?.length) return 0;
    ...
    for (const assignment of this.assignmentReport.singleAssignmentReports) {
      const userMarkObj = assignment.studentMarks?.find((stu: any) => stu.student.id === this.currentUser?.id);
      if (userMarkObj && typeof userMarkObj.mark === 'number') {
        total += userMarkObj.mark;
        count++;
      }
  ...
  getUserQuizCompletionRate(): string {
    ...
    const userQuizzes = this.quizReport?.singleQuizReports?.filter(q => q.students?.some((stu: any) => stu.id === this.currentUser?.id)) || [];
    return userQuizzes.length ? '100%' : '0%';
  }

  getUserAssignmentCompletionRate(): string {
    const userAssignments = this.assignmentReport?.singleAssignmentReports?.filter(a => a.studentMarks?.some((stu: any) => stu.student.id === this.currentUser?.id)) || [];
    return userAssignments.length ? '100%' : '0%';
  }

  getUserQuizCount(): number {
    return this.quizReport?.singleQuizReports?.filter(q => q.students?.some((stu: any) => stu.id === this.currentUser?.id)).length || 0;
  }
  ...
  getUserAssignmentCount(): number {
    return this.assignmentReport?.singleAssignmentReports?.filter(a => a.studentMarks?.some((stu: any) => stu.student.id === this.currentUser?.id)).length || 0;
  }
