...
import { profileFormConfig, profileFormControls } from '../settings-form.config';
...
export class TabProfileComponent implements OnChanges {  @Input() user: User | null = null;
  ...
  constructor(
    ...
    this.usernameControl = profileFormControls.find(control => control.id === 'username')!;
    this.emailControl = profileFormControls.find(control => control.id === 'email')!;
  }
  ...
  saveProfile(e: Event): void {
    ...
    this.settingsService.updateProfile(username, avatarUrl)
      .then(() => {
  ...
  private onFileSelected(event: Event): void {
    ...
    if (input.files && input.files[0]) {
      ...
      if (!allowedTypes.includes(file.type)) {
        this.toastService.error('Only .jpg, .jpeg, .png or .webp files are allowed');
        return;
      }
  ...
  get currentImageUrl(): string {
    return this.uploadedImageUrl || this.user?.avatar || 'public/images/mock-user-image.jpg';
  }
  }
