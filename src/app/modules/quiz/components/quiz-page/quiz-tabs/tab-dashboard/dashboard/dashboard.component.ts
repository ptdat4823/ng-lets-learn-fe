...
export class DashboardComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    console.log('questionCCount', this.questionCount);
  }
  ...
  calculateDashboardData() {
    ...
    const gradedResponses = this.studentResponses.filter(r => this.getMark(r) !== null);
    const avgMark = gradedResponses.length > 0
      ? (gradedResponses.reduce((sum, r) => sum + (this.getMark(r) || 0), 0) / gradedResponses.length).toFixed(1)
      : '0';
    const topMark = gradedResponses.length > 0
      ? Math.max(...gradedResponses.map(r => this.getMark(r) || 0))
      : 0;
    const avgTime = this.getAverageTimeSpent();
    const completionRate = this.studentCount > 0 ? Math.round((attemptedCount / this.studentCount) * 100) : 0;
    ...
    this.stats = [
      ...
      { label: 'Top mark', value: topMark.toString() },
  ...
  private getMark(response: StudentResponse): number | null {
    // For quiz, mark is usually calculated from answers
    if (response.data && Array.isArray((response.data as any).answers)) {
      return (response.data as any).answers.reduce((sum: number, ans: any) => sum + (ans.mark || 0), 0);
    }
    return null;
  }
  ...
  private getAverageTimeSpent(): string {
    ...
    const times = this.studentResponses
      .map(r => {
        const started = (r.data as any)?.startedAt ? new Date((r.data as any).startedAt).getTime() : null;
        const completed = (r.data as any)?.completedAt ? new Date((r.data as any).completedAt).getTime() : null;
  ...
  private calculateFileTypeSegments() {
    ...
    const colors = ['#E040FB', '#FF5252', '#448AFF', '#4CAF50', '#FFC107', '#FF9800'];
    this.fileTypeSegments = Object.entries(typeCounts).map(([type, count], idx) => ({
      value: count,
  ...
  private calculateGradedAssignmentsSegments() {
    ...
    const graded = this.studentResponses.map(r => this.getMark(r)).filter(m => m !== null) as number[];
    const ranges = [
    ...
    this.gradedAssignmentsSegments = ranges.map(range => {
      ...
      const count = graded.filter(m => m >= range.min! && m <= range.max!).length;
      return {
        value: count,
        color: range.color,
        label: range.label
